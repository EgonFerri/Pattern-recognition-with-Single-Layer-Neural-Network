    ...
    for num_target = 1:10
        for la = [0.0, 1.0, 10.0]
            for isd = [1,3]            
                ...
                if iheader == 1 fprintf(fileID,'num_target;   la; isd; typealmax; niter;     tex; te_acc;\n'); end
                fprintf(fileID,'         %1i; %4.1f;   %1i; %1i; %4i; %7.4f;  %5.1f;\n', mod(num_target,10), la, isd,typealmax, niter, tex, te_acc);
                iheader=0;
            end
        end
    end
